...
- **No existing domain or hosting account.** Target domain is `caleblim.com` (subject to availability check at deploy time; registrar TBD — Cloudflare Registrar still the cheapest option but not locked). Hosting on **Vercel** (amended 2026-05-18 — was Cloudflare Pages; Vercel chosen for simpler GitHub-import flow and built-in Astro auto-detection).
...
| Astro (code) over Framer (no-code) | Caleb is comfortable with markdown + git, removing Framer's main draw (visual editor for non-coders). Astro avoids platform lock-in (no HTML export from Framer), gets $0 hosting on Vercel's Hobby tier (was Cloudflare Pages — amended 2026-05-18), and has a cleaner build-time PDF rasterization pipeline. Both platforms can hit the same visual ceiling. | — Pending |
